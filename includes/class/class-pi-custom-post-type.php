 ...
 * @subpackage Pi_Custom_Post_type/includes
...
class Pi_Custom_Post_Type {
     ...
     * @since    1.0.0
     * @access   protected
     * @var      Pi_Photography_loader    $loader    Maintains and registers all hooks for the theme.
     */
    protected $loader;
    /**
     * Post type name.
     ...
     */
    public function __construct( $post_type_names, $options , $name ) {
        ...
        $this->loader->add_action( 'after_switch_theme', $this, 'flush' );
        // Register exisiting taxonomies.
        $this->loader->add_action( 'init', $this, 'register_exisiting_taxonomies' );
        // Add taxonomy to admin edit columns.
        $this->loader->add_filter( 'manage_edit-' . $this->post_type_name . '_columns', $this, 'add_admin_columns' );
        // Populate the taxonomy columns with the posts terms.
        $this->loader->add_action( 'manage_' . $this->post_type_name . '_posts_custom_column', $this, 'populate_admin_columns', 10, 2 );
        // Add filter select option to admin edit.
        $this->loader->add_action( 'restrict_manage_posts', $this, 'add_taxonomy_filters' );
        // rewrite post update messages
        $this->loader->add_filter( 'post_updated_messages', $this, 'updated_messages' );
        // bulk post and update messages
        $this->loader->add_filter( 'bulk_post_updated_messages', $this, 'bulk_updated_messages', 10, 2 );
    ...
    public function register_post_type() {
        ...
        // Default labels.
        $labels = array(
            'name'               => sprintf( __( '%s', $this->theme_name ), $plural ),
            'singular_name'      => sprintf( __( '%s', $this->theme_name ), $singular ),
            'menu_name'          => sprintf( __( '%s', $this->theme_name ), $plural ),
            'all_items'          => sprintf( __( '%s', $this->theme_name ), $plural ),
            'add_new'            => __( 'Add New', $this->theme_name ),
            'add_new_item'       => sprintf( __( 'Add New %s', $this->theme_name ), $singular ),
            'edit_item'          => sprintf( __( 'Edit %s', $this->theme_name ), $singular ),
            'new_item'           => sprintf( __( 'New %s', $this->theme_name ), $singular ),
            'view_item'          => sprintf( __( 'View %s', $this->theme_name ), $singular ),
            'search_items'       => sprintf( __( 'Search %s', $this->theme_name ), $plural ),
            'not_found'          => sprintf( __( 'No %s found', $this->theme_name ), $plural ),
            'not_found_in_trash' => sprintf( __( 'No %s found in Trash', $this->theme_name ), $plural ),
            'parent_item_colon'  => sprintf( __( 'Parent %s:', $this->theme_name ), $singular )
        );
    ...
    function register_taxonomy($taxonomy_names, $options = array()) {
        ...
        $labels = array(
            ...
            'singular_name'              => sprintf( __( '%s', $this->theme_name ), $singular ),
            'menu_name'                  => sprintf( __( '%s', $this->theme_name ), $plural ),
            'all_items'                  => sprintf( __( 'All %s', $this->theme_name ), $plural ),
            'edit_item'                  => sprintf( __( 'Edit %s', $this->theme_name ), $singular ),
            'view_item'                  => sprintf( __( 'View %s', $this->theme_name ), $singular ),
            'update_item'                => sprintf( __( 'Update %s', $this->theme_name ), $singular ),
            'add_new_item'               => sprintf( __( 'Add New %s', $this->theme_name ), $singular ),
            'new_item_name'              => sprintf( __( 'New %s Name', $this->theme_name ), $singular ),
            'parent_item'                => sprintf( __( 'Parent %s', $this->theme_name ), $plural ),
            'parent_item_colon'          => sprintf( __( 'Parent %s:', $this->theme_name ), $plural ),
            'search_items'               => sprintf( __( 'Search %s', $this->theme_name ), $plural ),
            'popular_items'              => sprintf( __( 'Popular %s', $this->theme_name ), $plural ),
            'separate_items_with_commas' => sprintf( __( 'Seperate %s with commas', $this->theme_name ), $plural ),
            'add_or_remove_items'        => sprintf( __( 'Add or remove %s', $this->theme_name ), $plural ),
            'choose_from_most_used'      => sprintf( __( 'Choose from most used %s', $this->theme_name ), $plural ),
            'not_found'                  => sprintf( __( 'No %s found', $this->theme_name ), $plural ),
        );
                    ...
                    if ( is_array( $this->taxonomies ) ) {
                        ...
                        foreach( $this->taxonomies as $tax ) {
                            ...
                            if( $tax !== 'category' && $tax !== 'post_tag' ) {
                                ...
                                $new_columns[ $tax ] = sprintf( __( '%s', $this->theme_name ), $taxonomy_object->labels->name );
                            }
                        }
                    }
                }
        ...
        switch( $column ) {
            ...
            case ( taxonomy_exists( $column ) ) :
                ...
                // If no terms found.
                    ...
                    // Echo no terms.
                    printf( __( 'No %s', $this->theme_name ), $taxonomy_object->labels->name );
                }
    ...
    */
            ...
            if ( ! empty( $filters ) ) {
                ...
                foreach ( $filters as $tax_slug ) {
                    ...
                    if ( $terms ) {
                        ...
                        // Default show all.
                        printf( '<option value="0">%s</option>', sprintf( __( 'Show all %s', $this->theme_name ), $tax->label ) );
    ...
    function sortable( $columns = array() ) {
        ...
        $this->loader->add_filter( 'manage_edit-' . $this->post_type_name . '_sortable_columns', $this, 'make_columns_sortable' );

        // Run action that sorts columns on request.
        $this->loader->add_action( 'load-edit.php', $this, 'load_edit' );
    }
     ...
     * Set theme_name
     *
     * @param string $theme_name theme_name used for translation.
     */
    function set_theme_name( $theme_name ) {
    ...
    function updated_messages( $messages ) {
        ...
        $messages[$this->post_type_name] = array(
            0 => '',
            1 => sprintf( __( '%s updated.', $this->theme_name ), $singular ),
            2 => __( 'Custom field updated.', $this->theme_name ),
            3 => __( 'Custom field deleted.', $this->theme_name ),
            4 => sprintf( __( '%s updated.', $this->theme_name ), $singular ),
            5 => isset( $_GET['revision'] ) ? sprintf( __( '%2$s restored to revision from %1$s', $this->theme_name ), wp_post_revision_title( (int) $_GET['revision'], false ), $singular ) : false,
            6 => sprintf( __( '%s updated.', $this->theme_name ), $singular ),
            7 => sprintf( __( '%s saved.', $this->theme_name ), $singular ),
            8 => sprintf( __( '%s submitted.', $this->theme_name ), $singular ),
            9 => sprintf(
                __( '%2$s scheduled for: <strong>%1$s</strong>.', $this->theme_name ),
                date_i18n( __( 'M j, Y @ G:i', $this->theme_name ), strtotime( $post->post_date ) ),
                $singular
            ),
            10 => sprintf( __( '%s draft updated.', $this->theme_name ), $singular ),
        );
